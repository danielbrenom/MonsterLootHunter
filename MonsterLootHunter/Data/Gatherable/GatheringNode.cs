...
public partial class GatheringNode : IComparable<GatheringNode>
{
    ...
    public GatheringKind GatheringKind => (GatheringKind)BaseNodeData.GatheringType.Row;
                         ...
                         IReadOnlyDictionary<uint, List<uint>> gatheringItemPoint, GatheringPointBase node)
    {
        ...
        Name = nodeRow?.PlaceName.Value?.Name.ToString() ?? string.Empty;
        ...
        IntegralXCoord = coordRow != null ? MapUtils.NodeToMap(coordRow.X, nodeRow?.TerritoryType.Value?.Map.Value?.SizeFactor ?? 100f) : 100;
        IntegralYCoord = coordRow != null ? MapUtils.NodeToMap(coordRow.Y, nodeRow?.TerritoryType.Value?.Map.Value?.SizeFactor ?? 100f) : 100;
        ...
        Items = node.Item
                    .Select(i => gatherablesByGatherId.GetValueOrDefault((uint)i))
                    .OfType<Gatherable>()
                    .ToList();
        ...
        if (nodeRow?.TerritoryType.Value?.RowId <= 0)
            return;
